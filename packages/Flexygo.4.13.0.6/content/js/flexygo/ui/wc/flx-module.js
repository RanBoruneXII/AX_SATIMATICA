           ...
           */
            class FlxModuleElement extends HTMLElement {
              ...
              */
                    ...
                    if (buttons) {
                        ...
                        if (arrBtn.length > 0) {
                            ...
                            if (arrChildBtn.length > 0) {
                                this.setChildButtons(arrChildBtn, objectname, objectwhere, defString, reportname, this.reportwhere, processname);
                            }
                        }
                    }
                }
                setChildButtons(btns, objectname, objectwhere, defString, reportname, reportwhere, processname) {
                    let parentIds = [];
                    this.ctxMenusValues = btns.reduce((groupedBtns, btn) => {
                    ...
                    parentIds.forEach(id => {
                        let btn = $(this).find(`[buttonid="${id}"]`);
                        btn.addClass('hasSubmenus clickable');
                        if (btn.attr('data-type') === 'text') {
                            btn.append('<span class="caret"/>');
                            btn.click(() => {
                                this.printToolbarContextMenu(btn, id, objectname, objectwhere, defString, reportname, reportwhere, processname);
                            });
                        }
                        else {
                            let showMenuBtn = $(`<button subButtonid="${id}" class="${(btn[0] ? btn[0].className : 'btn btn-default')}"><span class="caret"/></button>`);
                            showMenuBtn.click(() => {
                                this.printToolbarContextMenu(showMenuBtn, id, objectname, objectwhere, defString, reportname, reportwhere, processname);
                            });
                            showMenuBtn.focus(() => {
                                btn = $(btn.selector);
                                btn.closest('span.submenuContainer').addClass('focused');
                            });
                            showMenuBtn.focusout(() => {
                                btn = $(btn.selector);
                                btn.closest('span.submenuContainer').removeClass('focused');
                            });
                ...
                printToolbarContextMenu(triggerElement, btnId, objectname, objectwhere, defString, reportname, reportwhere, processname) {
                ...
                saveModule(objectName, objectWhere, module, button, afterSaveGoTo = '', defaults = null) {

...
(function (flexygo) {
    class Process {
        ...
        */
            ...
            if (!cllback) {
                cllback = (response) => {
                    if (response) {
                        ...
                        if (response.JSCode) {
                            var func = new Function('processname', 'objectname', 'objectwhere', 'targetid', 'excludeHist', 'triggerElement', 'currentProcess', response.JSCode);
                            let el = this;
                            if (triggerElement && triggerElement[0]) {
                                el = triggerElement[0];
                            }
                            func.call(triggerElement[0], this.processName, this.objectName, this.objectWhere, targetid, excludeHist, triggerElement, this);
                        }
        ...
        /**
        * Execute the process asyncronously.
        * @method run
        * @param {string} processparams - Values to invoke the process.
        * @param {function} [cllback] - function invoked after process executed. If null, success or error message.
        * @param {string} [targetid] - the target Id. For navigation objects.
        * @param {string} [excludeHist] - Exclude history regs. For navigation process.
        * @param {object} [triggerElement] - The item that launches de process (Usually the button).
        * @param {boolean} jsBeforeExecuted - Indicates if a JS BeforeExec has been executed
        * @param {boolean} isLast - Indicates if is the last process to be called in an execution process chain
        */
        runAsync(processparams, cllback, targetid, excludeHist = true, triggerElement, jsBeforeExecuted = false, isBefore = false) {
            return new Promise((resolve, reject) => {
                var params = {
                    ...
                    "ObjectWhere": this.objectWhere,
                    "ProcessParams": processparams,
                    "JSBeforeExecuted": jsBeforeExecuted
                };
                if (!cllback) {
                    cllback = (response) => {
                        if (response) {
                            ...
                            if (!response.MoreProcesses && !isBefore) {
                                if (response.LastException && response.LastException.Message) {
                                    flexygo.msg.error(response.LastException.Message);
                                }
                                else if (response.WarningMessage) {

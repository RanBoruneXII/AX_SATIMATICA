...
(function (flexygo) {
    ...
    (function (nav) {
       ...
       */
        function execProcessAsync(processname, objectname, objectwhere, defaults, processparams, targetid, excludeHist, triggerElement, callBack, showProgress, jsBeforeExecuted = false, isBefore = false, originalProcess) {
            return new Promise((resolve, reject) => {
                ...
                if ((!proc.config.IsWorkflow) && proc.config.HasParams && processparams == null) {
                    ...
                    if (typeof proc.config.ConfirmText != 'undefined' && proc.config.ConfirmText && proc.config.ConfirmText != '') {
                        let resultCallback = (result) => {
                            if (result) {
                                proc.runAsync(processparams, callBack, target, excludeHist, triggerElement, jsBeforeExecuted, isBefore).then(() => {
                                    resolve();
                                });
                            }
                        };
        ...
        nav.execProcessAsync = execProcessAsync;
